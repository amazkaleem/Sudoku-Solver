...
#include "../../UI/Selection/SelectionHandler.h"
    ...
    void handleKeyboardInput(Board& board) {
        ...
        for (int key = KEY_ONE; key <= KEY_NINE; key++) {
            if (IsKeyPressed(key)) {
                ...
                if (selection.hasSelectedCell()) {
                    ...
                    // Only set if not original cell
                    if (!board.isCellOriginal(row, col)) {
                        board.setCell(row, col, number, false);
                    }
                }
            }
        }
        ...
        if (IsKeyPressed(KEY_DELETE) || IsKeyPressed(KEY_BACKSPACE)) {
            if (selection.hasSelectedCell()) {
                ...
                if (!board.isCellOriginal(row, col)) {
                    board.setCell(row, col, 0, false);  // ✅ Use 0 instead of EMPTY_CELL
                }
            }
        }
